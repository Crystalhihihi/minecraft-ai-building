...
**访谈闭环成立+额度归因(2026-08-01 深夜)**:用户实测完整访谈+建造两轮,反馈"有卡片明显快很多,消耗也变小很多"——风格卡同时是质量卡和成本卡(免探索轮次)。sessions.json 账单实证:正常建造 20-60k 全价输入+1-10k 输出/把,缓存命中 89-97%(烧钱三大头=轮次/渲染图/翻车重试,非缓存未命中)。教训:访谈 agent 曾"幻觉式提问"(以为问了其实没发)——ask_player 工具化根治;批量提问刷屏→单问题制;20s 切片误判超时→无限等待+广播指纹去重。部署事故:PCL 游戏进程是 java.exe 非 javaw,误判导致热替换 jar、游戏内类加载崩溃("每次失败加载的类都不一样"是特征症状)

## E9:AI 建造偏好重复性实验 A 组(带抽签器,plan-only×8)—— 中 1/3:风格轴全收敛,参数轴被抽签器撬开(2026-08-05)

> 方法:同一中性提示词"在草地上建一栋住得舒服的房子"连跑 8 次(dev server + RCON 驱动,kimi-code/k3-256k),每次只跑到 plan.md 落盘即 /aicancel(零方块放置),离线比对 8 份计划。数据/脚本:`scratch/ab_test/`(plan_s27..s34.md + order/brief + analyze.py + results.md)。

**流程**:每次 /aibuild → 访谈 agent 首问(深度题)后 RCON /aichat 跳过 → interviewer 跑 patterns/lottery.py 抽签 + 写 intake_brief.md(含任务单)→ 自动交接 builder → builder 读卡+勘察+propose_site+写 plan.md → 落盘即终止。单把 4-7 分钟,16-22 轮;合计全价输入 121k + 缓存读 2.52M + 输出 8k(首把冷缓存 43k,其余 8-13k/把)。

**四项指标**(阈值:①众数≥7/8 ②CV<10% ③Jaccard>0.7):

| 指标 | 数值 | 判定 |
| --- | --- | --- |
| ① 风格卡分布 | plains_cabin 8/8 = 100% | **中** |
| ② 尺寸 CV | L 14.3% / W 18.0% / H 8.3% / 高宽比 8.4% | 未中(L/W 超线) |
| ③ 材质 Jaccard | 均值 0.673,区间 [0.500, 0.889] | 未中(差 0.027) |
| ④ lottery 接线 | 8/8 有 build_order.json + brief 任务单 + plan 引用 | 接线生效 |

8 份计划骨架:全部单层双坡木屋(gable_roof.py 45° spruce_stairs overhang 1)、橡木/云杉体系、室内净高 3。footprint 7×9×5 + 5×7×2(+s29 实排 7×7 偏离任务单);墙高 3×6 + 4×2。

**判定:中 1/3 → 不构成强 mode collapse,但结构性收敛明显**:风格卡轴 100% 收敛(注意有语义成分——"草地+舒服的房子"选 plains_cabin 是合理匹配而非纯偏好);抽签器在卡内参数轴(footprint/墙板混比/门廊/烟囱/窗尺寸/texture_seed)上注入了真实多样性,把 ②③ 压到阈值线以下/附近。

**重大异常——抽签 seed 被日期锁死**:8 次中 5 次(s27/s29/s30/s33/s34)interviewer 用当天日期 20260805 作 lottery seed;抽签是 (style,seed) 确定性函数,同 seed 必同结果 → 8 次只产出 4 种 build_order,5 份完全相同(连 texture_seed 都一样)。另 3 次 seed=28/310805/42 各不相同。**抽签器接线 8/8 生效,但多样性被 agent 的 seed 习惯砍掉一大半**——同单组材质 Jaccard 0.729 vs 跨组 0.659 vs 异单组 0.561,层次证明轴差异确实传导到了材质。修复方向:lottery.py 默认 seed 改随机/会话号+时间戳,或手册禁止日期 seed。

**其他异常记录**:
- s26 假启动(作废):控制台槽位残留 s25 的旧选区,/aibuild 直接消费绑定 bounds → 立即取消重开(s27 起无此问题)。教训:实验前先 /aiselect clear
- 跳过送达:跳过消息在 agent 下个工具响应才送达,4/8 会话补发了第 2-3 次;未出现访谈没跳过成功的情况
- s29 builder 未严格执行任务单(抽签 7×9 实排 7×7);s30/s34 墙高自定 4(卡默认 3)
- 选址分化:anchor 是历史测试留下的高空石砖平台(非草地)——4 次就近用平台(计划补绿化),4 次远赴真草地(最远 y=-59 低地)。选址行为本身是未被抽签覆盖的自由度
- 假死判活未用上:8 把全程流畅,无一会话卡死

**B 组对照基线推断**:若 8 次全同(无抽签),①中 ②中(CV≈0) ③中(Jaccard→0.9+),3/3 强 collapse。A 组实测 1/3——抽签器把参数轴多样性救了回来,但风格轴和"单层双坡小木屋"骨架依然铁打不动。

## E13 · 账单定价错误修正 + 模型路由前后对比（2026-08-13，quant 审计）

- **定价错误**：账单粗估长期按 Kimi K2 刊例（¥4/¥16/¥1）计算，但 wire 日志 model 字段证明
  Kimi 档实际全程跑 K3（官方 ¥20/¥100/¥2）。按真实 token 重算（91 会话 07-30→08-10）：
  总成本 ¥144.23 → **¥345.28**（低估 2.39×）；59 个 K3 会话 2.74×；单栋均值 ¥2.11 → **¥4.84**；
  E2 庄园 ¥3.09 → **¥8.50**。全量证据见 docs/quant-audit-2026-08-13.md（70/82 会话逐 token 对账一致）。
  遗留：AgentRunner.roughCost 的分档常量仍是 K2 价，待改。
- **flash 路由前后**：E6（07-31）v4-flash 初测 2/2 建成但 35.3s/调用、77.9k token 假死、
  质量未采信（注意：非"完全不可用"，口径修正）；08-08 切换后 **14/14 成功、均值 7.9min/棵、
  6.2s/调用、¥0.24/棵**，比 K3 档快 2.2×、省约 96%。
- **封存后运行**：08-08 起 20 个会话（19 done）、935,181 方块、¥31.17，此前零文档记载。
- **规矩 16：账单/性能数字必须标定价版本与口径**；"模型不可用"类结论要回查日志再引。
