...
- **质量底线**(写进 AGENTS.md 施工规范):**MC 原生比例**(门高 2 格、层高 3-4 格、墙厚 1 格——禁止现实/教堂比例,这是"格格不入"的根源);比例不畸形(高宽比 1:1~4:1)、屋顶与墙体材质区分、开窗有节奏不随机、材料呼应地形(森林木/沙漠砂岩/雪地云杉)、底部与地形衔接不浮空不半埋。

### 5.5 模式库(AI 不发明形状,AI 组装形状)

AI 纯生成的上限是"结构正确的平庸"。好看的建筑 = 成熟模式 × 好参数 × 地形适配,所以形状也必须资产化:

- **模式 = 参数化生成器脚本 + 参数卡**,存工作目录 `patterns/`(如 `gable_roof.py`:跨度/坡度/出檐/材料;`crenellation.py`:垛口间距;`road_segment.py`:路拱/边沟/铺装渐变);跑在 AI 沙盒,是**内容资产**,mod 零代码;
- **首批 5-8 个手写**:AI 起草 + 对着渲染图人工调参,调到好看再固化,每个都"好看得稳定"而非追求覆盖;
- **来源**:MC 建筑教程(步骤本身就是模式配方)与无光影建筑比赛作品(无光影抽掉遮羞布,剪影/节奏/材料纪律是最本质的教材)——**看图学规律、手写生成器**,不解析不导入任何作品文件;学手法,不学作品,版权干净;
- **比例铁律**:所有模式必须 MC 原生比例(见 §5.4),拒绝现实比例;
- **地形改造模式**(如 `terraform_pad.py`):可用地形函数——smoothstep 衰减曲线做"台地→原地形"平滑过渡、Perlin 噪声做表面变化防死板,全部在沙盒脚本内实现;
- 风格卡片管材料,模式库管形状,两者正交组合。

### 5.6 环境上下文分层模型(永不给 AI 原始世界数据)

环境配合与地形改造的数据量必须有界,分层供给:

- **本地层(红线 ±32)**:全细节,`terrain.json` 高度图/水体/树木(spawn 前自动给);
- **街区层(±128)**:粗摘要——坡度分区、水体轮廓、既有建筑**包围盒+材料统计**(不给方块流);
- **视觉即通道**:周边环境感知的主力是 `render_region` 图片(≈1-2k token 顶十万块坐标),不是数据导出;
- **既有建筑配合**:同会话记忆 + render + `get_region_summary`(统计级),不做方块级读取;
- **改造环境(terraforming)**:限定"红线 + ≤16 格过渡带",由高度图驱动的生成器脚本执行(放坡/衔接/植被点缀);
- 铁律:**永远抽象 locally、压缩 visually、限定 geographically**——数据爆表只发生在错误的通道设计上。
...
- **强制自检**:AGENTS.md 把"渲染自检 N 轮"列为完工验收标准——AI 必须自己 render、自己挑刺(对照 §5.4 质量底线)、自己改,改够轮次才许宣布完工(轮数 Phase 6 实测后定);渲染图同时存一份到工作目录供玩家查看。
