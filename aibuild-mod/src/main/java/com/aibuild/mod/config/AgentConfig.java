 ...
 */
public record AgentConfig(String agentCommand, long idleTimeoutMinutes, long hardTimeoutMinutes, long tickBudgetMs) {
    ...
    public static AgentConfig load(Path gameDir) {
        ...
        if (!Files.isRegularFile(path)) {
            AgentConfig defaults = new AgentConfig(defaultCommand, DEFAULT_IDLE_TIMEOUT_MINUTES, DEFAULT_HARD_TIMEOUT_MINUTES, DEFAULT_TICK_BUDGET_MS);
            try {
        ...
        try {
            ...
            return new AgentConfig(command, idle, hard, budget);
        } catch (Exception e) {
            AiBuildMod.LOGGER.warn("[aibuild] failed to parse {}, using defaults", path, e);
            return new AgentConfig(defaultCommand, DEFAULT_IDLE_TIMEOUT_MINUTES, DEFAULT_HARD_TIMEOUT_MINUTES, DEFAULT_TICK_BUDGET_MS);
        }
    }
