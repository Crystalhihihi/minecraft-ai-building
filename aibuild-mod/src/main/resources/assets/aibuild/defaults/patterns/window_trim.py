...
docs/research/detail-techniques.md §2:
...
(shutters/flowerbox 已下线: 活板门全线禁用 — 实测 AI 用不明白, 2026-08-07)
...
protruding piece (sill/lintel) lands on a ring cell (projection=1),
the piece wins. All direction states (stair facing/half) are DERIVED by the
script — never hand-edit them in the output
...
def build(p):
    ...
    on = {k: bool(p[k]) for k in ("sill", "lintel")}
    ...
    # ---- sill: protruding ledge under the opening (§2.3.1) ----------------
    if on["sill"]:
